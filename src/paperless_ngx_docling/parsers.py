...
class DoclingParser:
    ...
    def __init__(self, logging_group: object = None) -> None:
        ...
        # Try to import the converter - will fail if optional dependency not installed
        try:
            from docling_serve.serve import DoclingRemoteConverter
            self.converter = DoclingRemoteConverter()
            logger.debug("Using DoclingRemoteConverter")
        except ImportError:
            try:
                # TODO: Implement local converter
                # from local.converter import DoclingLocalConverter
                # self.converter = DoclingLocalConverter()
                logger.error("Local converter not yet implemented")
                raise NotImplementedError("Local converter not yet implemented. Install with: pip install paperless-docling-parser[docling-serve]")
            except ImportError:
                logger.error("No converter available. Install with: pip install paperless-docling-parser[docling-serve] or paperless-docling-parser[local]")
                raise ImportError("No converter available. Install with: pip install paperless-docling-parser[docling-serve] or paperless-docling-parser[local]")
    ...
    def parse(self, document_path: Path, mime_type: str, *, produce_archive: bool = True) -> None:
        """
        Parse a document using the configured converter.
