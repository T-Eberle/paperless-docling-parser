...
This project uses **automated releases** triggered by PR labels when merging to the `main` branch.

### Version Labels (Required for PRs to main only)

**Every PR targeting the `main` branch must have exactly ONE version label:**

- **`version:patch`** - Bug fixes and minor changes (v0.1.0 → v0.1.1)
- **`version:minor`** - New features, backwards compatible (v0.1.0 → v0.2.0)
- **`version:major`** - Breaking changes (v0.1.0 → v1.0.0)

### How It Works

1. **Create your PR** to the `main` branch
2. **Add a version label** (exactly one):
   - For bug fixes: Add `version:patch` label
   - For new features: Add `version:minor` label
   - For breaking changes: Add `version:major` label
3. **Validation runs automatically**:
   - The `validate-version-label` workflow checks for exactly one version label
   - PR cannot be merged if validation fails (zero or multiple labels)
4. **Merge your PR**:
   - The `auto-tag` workflow reads the label
   - Creates appropriate version tag (e.g., v0.1.1, v0.2.0, v1.0.0)
   - Triggers the `publish-release` workflow
...
- **MAJOR** (v1.0.0): Incompatible API changes → Use `version:major`
- **MINOR** (v0.2.0): New functionality (backwards compatible) → Use `version:minor`
- **PATCH** (v0.1.1): Bug fixes (backwards compatible) → Use `version:patch`

### Examples

**Bug Fix PR:**
```
1. Create PR with bug fix
2. Add label: version:patch
3. Merge PR
4. Result: v0.1.0 → v0.1.1
```

**New Feature PR:**
```
1. Create PR with new feature
2. Add label: version:minor
3. Merge PR
4. Result: v0.1.0 → v0.2.0
```

**Breaking Change PR:**
```
1. Create PR with breaking changes
2. Add label: version:major
3. Merge PR
4. Result: v0.1.0 → v1.0.0
```
...
- The validation workflow will block merging if:
  - No version label is present
  - Multiple version labels are present
- You can change the version label before merging if needed
